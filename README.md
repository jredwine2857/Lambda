# GPU Cluster PoC Kit — 2-Node Lambda Cloud Lab

An end-to-end GPU proof-of-concept I can stand up, benchmark, and tear down in one
pipeline run: provision a 2-node GPU cluster, validate the interconnect with an NCCL
bandwidth test, run multi-node PyTorch DDP training, serve an LLM with vLLM, watch it all
in Prometheus/Grafana, and load-test it with Locust and vLLM's `benchmark_serving`.

## Why this shape

This mirrors how I run a customer GPU PoC: validate the fabric first, benchmark
training, benchmark inference under load, then hand the customer a report they can take
to their CTO. Each step in the repo is one stage of that process:

| Stage | What it proves |
|---|---|
| `infra/` provision 2 nodes | Repeatable, IaC-driven cluster build (Terraform + GitHub Actions) |
| `distributed/nccl_bandwidth_test.py` | Interconnect is healthy *before* anyone trusts a training number |
| `distributed/ddp_train.py` | Multi-node distributed training works and scales |
| `inference/vllm_serve.sh` + TTFT client | Inference latency (time-to-first-token) on real hardware |
| `monitoring/` Prometheus + Grafana | GPU and serving observability a customer would actually run in production |
| `load_test/` Locust + `benchmark_serving` | Throughput and latency percentiles under concurrent load |
| `BENCHMARK_REPORT_TEMPLATE.md` | The customer-facing PoC summary: what was measured, why, and where the bottleneck is |

**Deliberately out of scope at 2 nodes** (and where a production design goes next):
Slurm vs Kubernetes for multi-node orchestration, InfiniBand/RoCE vs plain Ethernet for
NCCL all-reduce at scale (see my
[RoCEv2 / NVMe-oF fabric design](https://github.com/jredwine2857/avd-roce-nvmeof-fabric)),
distributed storage (NFS, NVMe-oF, Weka, VAST), TensorRT-LLM as an alternative to vLLM,
and 3D parallelism / Megatron-LM for large-model training.
...
The workflow uses Terraform (`hashicorp/setup-terraform`) because it's what most
customers already run. OpenTofu is a near-identical drop-in if you'd rather avoid
...
The same API calls are wrapped in Terraform in `infra/terraform/` — a thin
`null_resource` wrapper, since Lambda doesn't ship an official first-party provider.
...
time, TTFT p50/p95, throughput at N concurrent users, GPU utilization from Grafana).
That document is the real deliverable of a PoC: what was measured, why, and what the
customer should expect their bottleneck to be at production scale.
